...
void loop() {
  if (millis() - pmillis >= 5000) {
    pmillis = millis();
    lcdUpdate();
    if (read_ultra(trig1, echo1) < 2.5 || read_ultra(trig2, echo2) < 2.5 ) {
      ObjectFlag = true;
...
void lcdUpdate() {
  ...
  lcd.setCursor(0, 0);
  lcd.print("Front: ");
  lcd.print(read_ultra(trig2, echo2), 2);
  lcd.print("m");
  lcd.setCursor(0, 1);
  lcd.print("Back: ");
...
void send_data(String object)
{
  if (Firebase.ready() && signupOK && (millis() - sendDataPrevMillis > 2500))
  {
    ...
    Firebase.RTDB.setString(&fbdo, "Parameters/carC_Flag", object);
  }
}
